...
results_path = r'C:\DEV\vsession_accuracy\Results\Geral_{}'.format(
    {'scale': 'linear', 'less_dist': 'proximidade', 'none': 'sem_compatibilizacao'}.get(
                    ...
                    if scale_ == vet_scale[0] and class_ == list(dic_pec_mm['H'])[0]:
                        ...
                        for p_ in ps_t:
                            ...
                            if PROG_METHOD == 'scale':
                                ...
                                # Progressiva pela projeção na ref — sem inversão.
                                dist_ = round(geom_r.lineLocatePoint(QgsGeometry(p_)), 2)
                            else:
                                dist_ = geom_t.lineLocatePoint(QgsGeometry(p_))
                                if ci:
